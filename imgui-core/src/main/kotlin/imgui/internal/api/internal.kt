...
internal interface internal {
    ...
    fun focusWindow(window: Window? = null) {
        ...
        val focusFrontWindow = window?.rootWindow ?: null // NB: In docking branch this is window->RootWindowDockStop
        val displayFrontWindow = window?.rootWindow ?: null

        // Steal focus on active widgets
        if (g.activeId != 0)
            g.activeIdWindow?.let {
                if (it.rootWindow !== focusFrontWindow)
                    clearActiveID()
            }
