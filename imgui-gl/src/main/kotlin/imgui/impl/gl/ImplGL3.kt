...
class ImplGL3 : GLInterface {
    ...
    fun setupRenderState(drawData: DrawData, fbWidth: Int, fbHeight: Int) {
        ...
        val clipOriginLowerLeft = when {
            CLIP_ORIGIN && Platform.get() != Platform.MACOSX && glGetInteger(GL_CLIP_ORIGIN) == GL20C.GL_UPPER_LEFT -> false
            else -> true
        }
        ...
        if (!clipOriginLowerLeft) { val tmp = T; T = B; B = tmp; } // Swap top and bottom if origin is upper left
